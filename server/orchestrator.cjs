...
async function routeOrchestrator(pathname, payload) {
  ...
  if (pathname === '/api/orchestrator/candidates') return isLiveMode() ? liveCandidates(payload) : { candidates: generateCandidates(payload) };
...
async function liveCandidates(payload) {
  ...
  if (!result.ok || !Array.isArray(result.value?.candidates)) return { candidates: fallback, providerFallback: result.errors || [], providerTrace: traceFromResult(result, true) };
  ...
  return candidates.length
    ? { candidates, provider: result.provider, model: result.model, providerTrace: traceFromResult(result, false) }
    : { candidates: fallback, providerFallback: ['No valid candidates returned.'], providerTrace: traceFromResult(result, true) };
}
